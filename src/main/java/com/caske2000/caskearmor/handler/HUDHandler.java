...
public class HUDHandler extends Gui
{
    ...
    private static void renderHUD()
    {
        if ((mc.inGameHasFocus || (mc.currentScreen != null && (mc.currentScreen instanceof GuiChat))) && !mc.gameSettings.showDebugInfo)
        {
            ...
            if (helmet != null && helmet.getItem() instanceof IItemRFUpgradable)
            {
                IItemRFUpgradable helmetProvider = (IItemRFUpgradable) helmet.getItem();
                drawToHUD(helmetProvider.getHUDString(helmet), 2);
            }
            ItemStack chest = mc.thePlayer.getCurrentArmor(2);
            if (chest != null && chest.getItem() instanceof IItemRFUpgradable)
            {
                IItemRFUpgradable chestProvider = (IItemRFUpgradable) chest.getItem();
                drawToHUD(chestProvider.getHUDString(chest), 12);
            }
            ItemStack legs = mc.thePlayer.getCurrentArmor(1);
            if (legs != null && legs.getItem() instanceof IItemRFUpgradable)
            {
                IItemRFUpgradable legsProvider = (IItemRFUpgradable) legs.getItem();
                drawToHUD(legsProvider.getHUDString(legs), 22);
            }
            ItemStack boots = mc.thePlayer.getCurrentArmor(0);
            if (boots != null && boots.getItem() instanceof IItemRFUpgradable)
            {
                IItemRFUpgradable bootsProvider = (IItemRFUpgradable) boots.getItem();
    ...
    {
        if (ConfigurationHandler.enableHUD)
        {
